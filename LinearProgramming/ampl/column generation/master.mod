...
#var q
var q{(s,t) in El_u, 1..n_path[s,t]} integer;
# 1 if the virtual node s is mapped to the physical node i
# todo: check whether we need to replace GEO[s] with Np
var m{s in Nl, i in GEO[s]};

param delta{(s,t) in El_u, i in Np, 1..n_path[s,t]} binary, default 0;
...
s.t. flowNodeMapping1 {(s,t) in El_u, i in Np: (s,i) in AE}:
    sum{path in 1..n_path[s,t]} delta[s,t,i,path] * q[s,t,path] <= m[s,i]
