...
s.t. reducedCost:
    reducedCostValue = -1 - sum{(i,j) in Ep,(s,t) in El_u} (phi2[i,j,stage]*f[i,j,s,t]) -  sum{(s1,i) in AE_b,(s,t) in El_u} (f[s1,i,s,t] * phi3[s,t,s1,i,stage]) - sum{(s,t) in El_u} f_val[s,t] * phi1[s,t,stage]
...
s.t. notMappedSame {(s,t) in El_u, i in Np: (s,i) in AE and (t,i) in AE}:
	f[s,i,s,t] + f[i,t,s,t] <=1;
...
#problem pricing: f, f_val, reducedCostValue, onePair, reducedCost, balance1, balance2, balance3, balance4, balance5, notMappedSame, oneFlow;