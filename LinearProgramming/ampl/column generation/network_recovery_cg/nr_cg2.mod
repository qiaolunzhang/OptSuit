...
var alpha{(s,t) in El_u, 1..n_path[s,t], k in K} binary;
...
s.t. linkCapacity {(i,j) in Ep_u,k in K}:
    ...
    sum{(s,t) in El_u, path in 1..n_path[s,t]: (i,j) in path_link[s,t,path] or (j,i) in path_link[s,t,path]} q[s,t,path,k] <= L
...
s.t. check_working_logical1 {(s,t) in El_u, k in K, path in 1..n_path[s,t]}:
    beta_log[s,t,k] >= alpha[s,t,path,k]
;

# phi4
s.t. check_working_logical2 {(s,t) in El_u, path in 1..n_path[s,t], k in K}:
    q[s,t,path,k] >= alpha[s,t,path,k]
;

# phi5
s.t. check_working_logical3 {(s,t) in El_u, path in 1..n_path[s,t], k in K}:
    beta_log[s,t,k]  + q[s,t,path,k] - 1 <= alpha[s,t,path,k]
;

s.t. check_working_logical4 {(s,t) in El_u, k in K, path in 1..n_path[s,t], (i,j) in path_link[s,t,path]: (i,j) in Ep}:
    alpha[s,t,path,k] <= beta[i,j,k]
;
